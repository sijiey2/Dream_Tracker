...
{
    public partial class frmDream4 : Form
    {
        ...
        //Load the Dream4 form
        private void frmDream4_Load(object sender, EventArgs e)
        {
            //
            ...
            cs.Open();
            cmd.CommandText = "Select * from tblSettingGoals Where DreamNum = 'Dream4'";
            cmd.Connection = cs;
            ...
            cs.Open();
            cmd.CommandText = "SELECT TOP 1 * FROM tblDream4 ORDER BY CountDays DESC";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT SumOfHours FROM tblDream4 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream4)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT AverageHours FROM tblDream4 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream4)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT Percentage FROM tblDream4 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream4)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            for (int dblCountDays = 1; dblCountDays <= double.Parse(txtCountDays.Text); dblCountDays++) {
                cs.Open();
                cmd.CommandText = "Select AddHours From tblDream4 Where CountDays = '" + dblCountDays + "' ";
                cmd.Connection = cs;
                dr = cmd.ExecuteReader();
                if (dr.Read())
                {
        ...
        {
            //Close the Dream4 form
            this.Close();
        }

        //Confirm Quit
        private void frmDream4_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            //Insert Data
            cs.Open();
            cmd.CommandText = "Insert into tblDream4 (CountDays, SumOfHours, AverageHours, Percentage, AddHours) Values ( '" + txtCountDays.Text + "', '" + txtSumOfHours.Text + "', '" + txtAverageHours.Text + "', '" + txtPercentage.Text + "', '" + dblAddHours + "') ";
            cmd.Connection = cs;
            cmd.ExecuteNonQuery();
