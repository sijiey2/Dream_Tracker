...
{
    public partial class frmDream2 : Form
    {
        ...
        //Load the Dream2 form
        private void frmDream2_Load(object sender, EventArgs e)
        {
            //
            ...
            cs.Open();
            cmd.CommandText = "Select * from tblSettingGoals Where DreamNum = 'Dream2'";
            cmd.Connection = cs;
            ...
            cs.Open();
            cmd.CommandText = "SELECT TOP 1 * FROM tblDream2 ORDER BY CountDays DESC";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT SumOfHours FROM tblDream2 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream2)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT AverageHours FROM tblDream2 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream2)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT Percentage FROM tblDream2 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream2)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            for (int dblCountDays = 1; dblCountDays <= double.Parse(txtCountDays.Text); dblCountDays++) {
                cs.Open();
                cmd.CommandText = "Select AddHours From tblDream2 Where CountDays = '" + dblCountDays + "' ";
                cmd.Connection = cs;
                dr = cmd.ExecuteReader();
                if (dr.Read())
                {
        ...
        {
            //Close the Dream2 form
            this.Close();
        }

        //Confirm Quit
        private void frmDream2_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            //Insert Data
            cs.Open();
            cmd.CommandText = "Insert into tblDream2 (CountDays, SumOfHours, AverageHours, Percentage, AddHours) Values ( '" + txtCountDays.Text + "', '" + txtSumOfHours.Text + "', '" + txtAverageHours.Text + "', '" + txtPercentage.Text + "', '" + dblAddHours + "') ";
            cmd.Connection = cs;
            cmd.ExecuteNonQuery();
