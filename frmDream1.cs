...
{
    public partial class frmDream1 : Form
    {
        ...
        //Load the Dream1 form
        private void frmDream1_Load(object sender, EventArgs e)
        {
            //
            ...
            cs.Open();
            cmd.CommandText = "Select * from tblSettingGoals Where DreamNum = 'Dream1'";
            cmd.Connection = cs;
            ...
            cs.Open();
            cmd.CommandText = "SELECT TOP 1 * FROM tblDream1 ORDER BY CountDays DESC";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT SumOfHours FROM tblDream1 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream1)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT AverageHours FROM tblDream1 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream1)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            cs.Open();
            cmd.CommandText = "SELECT Percentage FROM tblDream1 WHERE CountDays = (SELECT MAX(CountDays) FROM tblDream1)";
            cmd.Connection = cs;
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            for (int dblCountDays = 1; dblCountDays <= double.Parse(txtCountDays.Text); dblCountDays++) {
                cs.Open();
                cmd.CommandText = "Select AddHours From tblDream1 Where CountDays = '" + dblCountDays + "' ";
                cmd.Connection = cs;
                dr = cmd.ExecuteReader();
                if (dr.Read())
                {
        ...
        {
            //Close the Dream1 form
            this.Close();
        }

        //Confirm Quit
        private void frmDream1_FormClosing(object sender, FormClosingEventArgs e)
        {
            //Show Warrning Message
            if (MessageBox.Show("Are you sure you that want to close this form ?", "Confirm Exit", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
            {
                e.Cancel = true;
            }               
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            //Insert Data
            cs.Open();
            cmd.CommandText = "Insert into tblDream1 (CountDays, SumOfHours, AverageHours, Percentage, AddHours) Values ( '" + txtCountDays.Text + "', '" + txtSumOfHours.Text + "', '" + txtAverageHours.Text + "', '" + txtPercentage.Text + "', '" + dblAddHours + "') ";
            cmd.Connection = cs;
            cmd.ExecuteNonQuery();
